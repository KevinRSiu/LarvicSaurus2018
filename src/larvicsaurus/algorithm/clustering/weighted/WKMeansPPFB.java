...
public class WKMeansPPFB extends StaticClustering {
    ...
    public WKMeansPPFB(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
    ...
    public Collection<StandardEntity> getClusterEntities(int index) {
    	...
    	if((result == null || result.isEmpty()) && index>=0){
            ...
            for (int index = 0; index < this.clusterSize; index++) {
                ...
                for (StandardEntity entity : this.clusterEntitiesList.get(index)) {
                    ...
                    switch(entity.getStandardURN()){
                    	case BUILDING: 			priority = MEDIUM; break;
                    	case REFUGE: 			priority = HIGH; break;
                    	case HYDRANT: 			priority = MEDIUM_HIGH; break;
                    	case GAS_STATION: 		priority = HIGH; break;
                    	case ROAD: 				priority = MEDIUM_LOW; break;
                    	case CIVILIAN: 			priority = LOW; break;
                    	case AMBULANCE_TEAM: 	priority = LOW; break;
                    	case FIRE_BRIGADE: 		priority = MEDIUM_LOW; break;
                    	case POLICE_FORCE: 		priority = LOW; break;
                    	case AMBULANCE_CENTRE: 	priority = LOW; break;
                    	case FIRE_STATION: 		priority = MEDIUM_LOW; break;
