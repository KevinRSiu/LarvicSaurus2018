...
public class HKMeansPPPF extends StaticClustering {
    ...
    public HKMeansPPPF(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
        ...
        this.clusterSizeTop = wi.getEntitiesOfType(StandardEntityURN.REFUGE).size() <= 3 ? wi.getEntitiesOfType(StandardEntityURN.REFUGE).size() : (int) wi.getEntitiesOfType(StandardEntityURN.REFUGE).size() / 3;
 ...
 private void calcStandard(int repeat) {
        ...
        for (int index = 0; index < this.clusterSizeTop; index++) {
            ...
            do {
            	int choice = getChoice(top_entityList);
            	centerEntity = top_entityList.get(choice);  
        ...
        for(int index=0; index<this.clusterSizeTop; index++){
            ...
            for(int jndex = 0; jndex < this.clusterSizeBottom; jndex++){
            	StandardEntity centerEntity;
            	do {
            		int choice = getChoice(cur_entityList);
                	centerEntity = cur_entityList.get(choice);
            ...
            for (int i=0; i<repeat; i++){
            	...
            	for(StandardEntity entity: cur_entityList){
            		StandardEntity tmp = this.getNearEntityByLine(this.worldInfo, this.centerList.subList(index*this.clusterSizeBottom, index*this.clusterSizeBottom+clusterSizeBottom-1), entity);
