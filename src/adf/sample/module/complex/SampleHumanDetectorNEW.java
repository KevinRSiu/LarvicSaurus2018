...
public class SampleHumanDetectorNEW extends HumanDetector {
    ...
    public SampleHumanDetectorNEW(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
        ...
        for (StandardEntity next : this.worldInfo.getEntitiesOfType(AMBULANCE_TEAM, FIRE_BRIGADE, POLICE_FORCE)) {
            ...
            if (this.agentInfo.getID().getValue() != h.getID().getValue()) {
                ...
                if (positionEntity != null && h.isHPDefined() && h.isBuriednessDefined()) {
                    if (h.getHP() > 2000 || h.getBuriedness() > 0) {
                        rescueTargets.add(h);
                    }
                }
            }
